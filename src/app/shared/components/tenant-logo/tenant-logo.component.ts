...
export class TenantLogoComponent {
  ...
  private dimensions() {
    switch (this.size()) {
      case 'sm': return { square: 'h-7 w-7',  full: 'h-7 w-auto max-h-7',  fullChip: 'h-7 px-2.5' };
      case 'lg': return { square: 'h-10 w-10', full: 'h-10 w-auto max-h-10', fullChip: 'h-10 px-3' };
      case 'xl': return { square: 'h-12 w-12', full: 'h-12 w-auto max-h-12', fullChip: 'h-12 px-3.5' };
      default:   return { square: 'h-9 w-9',  full: 'h-9 w-auto max-h-9',  fullChip: 'h-9 px-3' };
    }
  }
