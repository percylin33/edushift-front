...
@Component({
  ...
  template: `
    <span class="badge" [class]="cssClass()" [attr.title]="title()">
      <span class="mr-1.5 h-1.5 w-1.5 rounded-full" [class]="dotClass()"></span>
      {{ label() }}
    </span>
  `,
  styles: [
    `
      ...
      :host-context(.dark) .badge--active {
        background: rgba(34 197 94 / 0.15);
        color: rgb(187 247 208);
        border-color: rgba(34 197 94 / 0.3);
      }
      :host-context(.dark) .badge--leave {
        background: rgba(234 179 8 / 0.15);
        color: rgb(253 224 71);
        border-color: rgba(234 179 8 / 0.3);
      }
      :host-context(.dark) .badge--suspend {
        background: rgba(220 38 38 / 0.18);
        color: rgb(252 165 165);
        border-color: rgba(220 38 38 / 0.3);
      }
      :host-context(.dark) .badge--terminal {
...
export class EmploymentStatusBadgeComponent {
  ...
  protected readonly title = computed(() => {
    switch (this.status()) {
      case EmploymentStatus.Active:
        return 'Docente activo, asignable a secciones';
      case EmploymentStatus.OnLeave:
        return 'En licencia temporal';
      case EmploymentStatus.Suspended:
        return 'Suspendido — no puede iniciar sesión';
      case EmploymentStatus.Resigned:
        return 'Renunció';
      case EmploymentStatus.Retired:
        return 'Jubilado';
