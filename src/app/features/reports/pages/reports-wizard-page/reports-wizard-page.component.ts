 ...
 */
@Component({
  selector: 'edushift-reports-wizard-page',
  standalone: true,
  ...
  template: `
    <section class="mx-auto max-w-3xl px-4 py-8 sm:py-10">
      ...
      <!-- Step 1: type -->
      <fieldset class="mt-6 rounded-2xl bg-white dark:bg-slate-900 shadow
      ...
      <!-- Step 2: format -->
      <fieldset class="mt-4 rounded-2xl bg-white dark:bg-slate-900 shadow
                       ring-1 ring-slate-200 dark:ring-slate-800 p-5">
        <legend class="px-2 text-sm font-semibold">2. Formato</legend>
        <div class="mt-3 flex flex-wrap gap-2">
          @for (f of formats; track f) {
            <label class="flex items-center gap-2 rounded-lg border
                          border-slate-200 dark:border-slate-700 px-4 py-2
                          cursor-pointer hover:border-emerald-500
                          has-[:checked]:bg-emerald-50
                          dark:has-[:checked]:bg-emerald-900/20">
              <input type="radio" name="format"
                     [value]="f" [(ngModel)]="format"
                     class="text-emerald-600"/>
      ...
      <fieldset class="mt-4 rounded-2xl bg-white dark:bg-slate-900 shadow
        ...
        <div class="mt-3 flex items-center gap-3">
          ...
          @if (job()?.status === 'DONE') {
            <a [href]="downloadHref()"
               class="rounded-lg border border-emerald-600 text-emerald-700
                      dark:text-emerald-400 px-4 py-2 text-sm font-semibold
...
export class ReportsWizardPageComponent implements OnInit {
  ...
  private pollWithBackoff(publicUuid: string): void {
    ...
    const tick = () => {
      ...
      this.pollSub = this.api.get(publicUuid)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe({
          next: (j) => {
            this.job.set(j);
