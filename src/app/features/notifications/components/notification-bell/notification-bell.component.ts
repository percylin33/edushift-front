 ...
 */
@Component({
  selector: 'edushift-notification-bell',
  standalone: true,
  ...
  template: `
    <div class="relative">
      ...
      >
        ...
        @if (unreadCount() > 0) {
          <span class="absolute -top-1 -right-1 inline-flex h-5 min-w-5 px-1
                       items-center justify-center rounded-full
                       bg-rose-500 text-[10px] font-semibold text-white">
        ...
        @if (!realtime.connected() && !hideRealtimeDot()) {
          <span class="absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full
                       bg-amber-400 ring-2 ring-white dark:ring-slate-900"
                title="Realtime desconectado — recargando al volver"></span>
        }
      </button>

      @if (open()) {
        <div class="absolute right-0 z-30 mt-2 w-80 sm:w-96
          ...
          <div class="max-h-96 overflow-y-auto">
            @if (loading() && items().length === 0) {
              ...
              <div class="px-4 py-12 text-center">
                <div class="mx-auto h-10 w-10 rounded-full bg-emerald-50
                  ...
                  <li>
                    <button type="button"
                            class="w-full text-left px-4 py-3 hover:bg-slate-50
                                   dark:hover:bg-slate-800 flex items-start gap-3"
                            [class.bg-emerald-50]="!n.readAt"
                            [class.dark:bg-emerald-900\/20]="!n.readAt"
                            (click)="onItemClick(n)">
                      <span class="mt-1 h-2 w-2 rounded-full flex-shrink-0"
                            [class.bg-emerald-500]="!n.readAt"
                            [class.bg-slate-300]="!!n.readAt"></span>
                      <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-slate-900 dark:text-slate-100
                                  line-clamp-1">{{ n.title }}</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">
                          {{ n.body }}
                        </p>
                        <p class="mt-1 text-[10px] uppercase tracking-wide
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  private onRealtime(p: { notificationUuid: string; subject: string | null;
                          templateKey: string; category: string; occurredAt: string; }): void {
